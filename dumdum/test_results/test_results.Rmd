...
## These tests should produce outputs
...
knitr::kable(head(test4))

```

### Test 5 
```{r test5, echo= FALSE, results='asis'}
test5 <- dummify(iris, "Species", dumNames = c("set" = "setosa","veris" = "versicolor", "vriginia"= "virginica"))

knitr::kable(head(test5))

```

### Test 6

```{r test6, echo= FALSE, results='asis'}
iris$Species<-as.character(iris$Species)
test6 <- dummify(iris,"Species")

knitr::kable(head(test6))

```

### Test 7
```{r test7, echo= FALSE, results='asis'}
ibis<-iris
ibis$Species<-as.integer(ibis$Species)
test7 <- test1 <- dummify(iris,"Species")


knitr::kable(head(test7))

```

### Test 8
The html won't go as far as the actual data, so here's what the new columns should look like
```{r test8, echo= FALSE, results='asis'}
...
knitr::kable(head(test8[,12:23]))

```

### Test 9
```{r test9, echo= FALSE, results='asis'}
mtcars$cyl<-as.integer(mtcars$cyl)
mtcars$gear <- as.factor(mtcars$gear)
mtcars$carb <- as.character(mtcars$carb)
...
knitr::kable(head(test8[,12:20]))

```

## These should Fail
### Test 10
```{r test10, echo= FALSE, results='asis'}
test10 <- dummify(mtcars, c("cyl","gear","carb"))

```
### Test 11
```{r test11, echo= FALSE, results='asis'}
test11 <- dummify(mtcars, "drat")
```
### Test 12
```{r test12, echo= FALSE, results='asis'}
test12 <- dummify(mtcars, cyl)
```
### Test 13
```{r test13, echo= FALSE, results='asis'}
test13 <- dummify_across(mtcars, c("cyl","gear","carb"), reference = 7)
```
### Test 14
```{r test14, echo= FALSE, results='asis'}
